...
gulp.task('initial-build-complete', (cb) => {   
    notifier.notify({
        ...
        icon: path.join(__dirname, 'web/public/assets/images/image.png'),
        title: 'Bundler',
...
gulp.task('build-bundle-complete', (cb) => {   notifier.notify({
    icon: path.join(__dirname, 'web/public/assets/images/Image.png'),
    title: 'Bundler',
    message: 'Bundled!',
    sound: 'Pop'   
...
gulp.task('sass', (cb) => {   gulp.src(SASS)
...
gulp.task('watch-and-alert', () => {   browserSync.init({
    ui: {
        port: 8079,
},
    port: 8080,
...
gulp.watch(['./web/src/**/*.tsx','./web/src/**/*.ts'], gulp.series('build-bundle-and-alert-with-logs', 'reload-page'))   

gulp.watch(['./web/src/**/*.scss'], gulp.series('build-sass-and-alert-with-logs', 'reload-page')   )})
