...
export const growthModel: IModelData[]  = [
    {symbol: 'VXUS', cusip: '921909768', weight: '00', color: colors.breakerBay},
    {symbol: 'IJR', cusip: '464287804', weight: '20', color: colors.lightForestGreen},
...
export const mapModelToPositionObject = (modelArray: IModelData[]): IPosition[] => {
    let positionArray: IPosition[] = []
    ...
    modelArray.forEach((model => {
        let formattedPosition = {
            ...
            value: Number(model.weight),
        }
        
        if (Number(model.weight) > 0) {
            positionArray.push(formattedPosition)
        }
